...
Spring Data unwraps a Vavr `Seq`, `Set`, or `Map` argument to a Java collection before the store sees it, so a derived query such as `findByStatusIn(io.vavr.collection.List<Status> statuses)` binds as a regular `IN` clause.
A Vavr `Option` argument is unwrapped to its value (or `null`), not to `java.util.Optional`.
