...
/**
 * Abstract class for Web&WebFLux PageableHandlerMethodArgumentResolver
 *
 * @author Eugene Utkin
 * @see org.springframework.data.web.PageableHandlerMethodArgumentResolver
 * @see org.springframework.data.webflux.PageableHandlerMethodArgumentResolver
 */
public class AbstractPageableHandlerMethodArgumentResolver {
    ...
    protected HandlerMethodArgumentResolver sortResolver;
    protected String pageParameterName = DEFAULT_PAGE_PARAMETER;
    protected String sizeParameterName = DEFAULT_SIZE_PARAMETER;
    ...
    protected Pageable resolvePageableParameter(
            MethodParameter methodParameter,
            String pageParameterAsString,
            String pageSizeParameterAsString,
            Sort sort
    ) {
        Optional<Pageable> defaultOrFallback = getDefaultFromAnnotationOrFallback(methodParameter).toOptional();
        Optional<Integer> page = parseAndApplyBoundaries(pageParameterAsString, Integer.MAX_VALUE, true);
        Optional<Integer> pageSize = parseAndApplyBoundaries(pageSizeParameterAsString, maxPageSize, false);
     ...
     * @return whether the given {@link Pageable} is the fallback one.
     * @since 1.9
     */
     ...
     * @return {@literal Optional.empty()} if parameter is empty else Optional with int value
     */
