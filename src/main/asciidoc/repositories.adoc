...
interface UserRepository extends CrudRepository<User, String>,
  ...
  default public void customize(QuerydslBindings bindings, QUser root) {
    ...
    bindings.bind(String.class).first((StringPath path, String value)        <3>
        -> path.containsIgnoreCase(value));
    bindings.excluding(root.password);                                       <4>
  }
}
----
...
<3> Define the default binding for `String` properies to be a case insensitive contains match.
<4> Exclude the _password_ property from `Predicate` resolution.
====
