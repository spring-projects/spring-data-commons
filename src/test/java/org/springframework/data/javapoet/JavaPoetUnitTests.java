...
class JavaPoetUnitTests {
	...
	void shouldRenderConditionalLongReturn() {
		...
		block = LordOfTheStrings.returning(Long.class).whenBoxed(Long.class, "$T.valueOf(1)", Long.class).otherwise("else")
				.build();
		assertThat(block).hasToString("return java.lang.Long.valueOf(1)");
	...
	void shouldRenderConditionalIntReturn() {
		...
		CodeBlock block = LordOfTheStrings.returning(Integer.class).whenBoxed(long.class, "$T.valueOf(1)", Long.class)
				.otherwise("else").build();
		assertThat(block).hasToString("return else");

		block = LordOfTheStrings.returning(Integer.class).whenBoxedInteger("$T.valueOf(1)", Integer.class).otherwise("else")
				.build();
	...
	void shouldRenderConditionalBooleanReturn() {
		...
		CodeBlock block = LordOfTheStrings.returning(boolean.class).whenBoolean("$L", true).otherwise("else").build();
		assertThat(block).hasToString("return true");

		block = LordOfTheStrings.returning(Boolean.class).whenBoolean("$L", true).otherwise("else").build();
		assertThat(block).hasToString("return true");

		block = LordOfTheStrings.returning(String.class).whenBoolean("$L", true).otherwise("else").build();
		assertThat(block).hasToString("return else");
	}
	...
	void shouldRenderConditionalNumericReturn() {
		...
		CodeBlock block = LordOfTheStrings.returning(boolean.class).number("someNumericVariable").otherwise("else").build();
		assertThat(block).hasToString("return else");

		block = LordOfTheStrings.returning(long.class).number("someNumericVariable").otherwise("else").build();
		...
		block = LordOfTheStrings.returning(int.class).number("someNumericVariable").otherwise("else").build();
		assertThat(block).hasToString("return someNumericVariable != null ? someNumericVariable.intValue() : 0");

		block = LordOfTheStrings.returning(Integer.class).number("someNumericVariable").otherwise("else").build();
		assertThat(block).hasToString("return someNumericVariable != null ? someNumericVariable.intValue() : null");

		block = LordOfTheStrings.returning(String.class).number("someNumericVariable").otherwise("else").build();
		assertThat(block).hasToString("return else");
	}
