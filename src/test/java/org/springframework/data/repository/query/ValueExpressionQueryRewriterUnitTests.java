...
class ValueExpressionQueryRewriterUnitTests {
	...
	void shouldEvaluateExpression() throws Exception {
		...
		QueryMethodValueEvaluationContextProviderFactory factory = new QueryMethodValueEvaluationContextProviderFactory(
				environment, QueryMethodEvaluationContextProvider.DEFAULT);

		Method method = ValueExpressionQueryRewriterUnitTests.MyRepository.class.getDeclaredMethod("simpleExpression",
				String.class);
		var extractor = rewriter.withEvaluationContextProvider(factory).parse("SELECT :#{#value}, :${foo}",
	...
	void shouldAllowNullValues() throws Exception {
		...
		Method method = ValueExpressionQueryRewriterUnitTests.MyRepository.class.getDeclaredMethod("simpleExpression",
				String.class);
		var extractor = rewriter.withEvaluationContextProvider(factory).parse("SELECT :#{#value}",
				new DefaultParameters(ParametersSource.of(method)));
